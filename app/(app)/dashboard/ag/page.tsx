...
export default function AdminAGPage() {
    ...
    const executeDelete = async () => {
        ...
        setIsSaving(true);
        const supabase = createClient();
        const { data: oldData } = await supabase.from('assemblee_generale').select('*').eq('id', deleteConfirm.id).single();
        await supabase.from('assemblee_generale').delete().eq('id', deleteConfirm.id);
        setCards(cards.filter(c => c.id !== deleteConfirm.id));
        await deleteNotificationByEntity(deleteConfirm.id);
        if (user) await logAction('Suppression', user.id, `${user.prenom} ${user.nom}`, user.email, `Suppression case AG: ${deleteConfirm.name || 'Sans titre'}`, oldData, null);
        setIsSaving(false);
    ...
    const handleSave = async (card: DocCard) => {
        ...
        const { data: oldData } = await supabase.from('assemblee_generale').select('*').eq('id', card.id).single();
        ...
        const { error } = await supabase.from('assemblee_generale').update(updatedData).eq('id', card.id);

        if (!error) {
            toast({ title: "Case enregistrée" });
            setCards(cards.map(c => c.id === card.id ? { ...c, isSaving: false, fileToUpload: null, url: urlTarget } : c));
            if (user) {
                await logAction('Modification', user.id, `${user.prenom} ${user.nom}`, user.email, `Mise à jour case AG: ${card.titre}`, oldData, updatedData);
                if (card.titre && card.type !== 'empty') {
                    await createNotification("Document AG", `Nouveau document: ${card.titre}`, "/ag", "ag", card.id);
                }
            }
        } else {
