...
export default function AdminAGPage() {
    ...
    const fetchCards = async () => {
        ...
        if (loadedCards.length < 6) {
            ...
            const { data: insertedData, error: insErr } = await supabase.from('assemblee_generale').insert(newInserts).select();
            if (insertedData) {
            ...
            <Card className="shadow-lg border-2 border-primary/10 overflow-hidden relative">
                ...
                <CardHeader>
                    <div className="flex justify-between items-center z-10 relative">
                        <div>
                            <CardTitle className="text-3xl font-extrabold text-primary">Gestion AG</CardTitle>
                            <CardDescription className="text-base mt-2 max-w-2xl font-medium">
                                Ajoutez, renommez, supprimez et <b>réorganisez</b> vos documents d'AG comme bon vous semble !
                            </CardDescription>
                        </div>
                    </div>
                </CardHeader>
